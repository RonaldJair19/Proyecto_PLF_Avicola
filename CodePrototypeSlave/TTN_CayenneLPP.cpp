...
uint8_t TTN_CayenneLPP::addGasToxic(uint8_t channel, uint16_t gas)
{
    if ((cursor + LPP_GAS_TOXIC_SIZE) > maxsize)
    {
        return 0;
    }
    //buffer[cursor++] = channel;
    //buffer[cursor++] = LPP_GAS_TOXIC;
    buffer[cursor++] = gas >> 8;
    buffer[cursor++] = gas;
...
uint8_t TTN_CayenneLPP::addGasFlamable(uint8_t channel, uint16_t gasFlamable)
{
    if ((cursor + LPP_GAS_FLAMABLE_SIZE) > maxsize)
    {
        return 0;
    }
    //buffer[cursor++] = channel;
    //buffer[cursor++] = LPP_GAS_FLAMABLE;
    buffer[cursor++] = gasFlamable >> 8;
    buffer[cursor++] = gasFlamable;
